    ...
    <div class="row">
        <div class="col-md-8">
            <div class="panel panel-default">
                <div class="panel-heading">Tambah Detil TNA</div>
                <div class="panel-body">
                    <form role="form" class="form-horizontal" action="/detil_tna" method="post">
                        <div class="form-group{{ $errors->has('detil_tna') ? ' has-error' : '' }}">
                            <label for="detil_tna" class="control-label col-sm-2">Detil TNA:</label>
                            <div class="col-sm-10">
                              <textarea rows="4" cols="50" class="form-control" id="detil_tna" type="text" name="detil_tna"  placeholder="Masukkan detil TNA">{{old('detil_tna')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('kategori_tna_id') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-4">
                              <select name="kategori_tna_id" class="form-control">
                                <option value="" @if(old('kategori_tna_id') == '')selected @endif></option>
                                @foreach ($kategoritna as $kategoritna)
                                  <option value="{{$kategoritna->id}}" @if(old('kategori_tna_id') == $kategoritna->id)selected @endif>{{$kategoritna->kategori_tna}}</option>
                                @endforeach
                              </select>
